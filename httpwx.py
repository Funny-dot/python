...
class httpwx:
    ...
    is_plugins=0
    port=00
    logdir=r'c:/web/python/update.log'
    maindir=r'c:/web/python/'
    duankou=r'C:.ide' #开发者工具IDE
    configdata = ['域名', 'appid', '小程序名称', '1.3.3', '简介', './private.key']
    def vs(self,dir):
        ...
        for f in filelist:
            ...
            if os.path.isfile(filepath):
                try:
                    ...
                    files.close()
                    filesd = open(filepath, 'w', encoding='utf-8')
                    filesd.write(rr)
                    filesd.close()
                except:
                    print('1')
    ...
    def updates(self):
        ...
        file = open(self.dir + '/project.config.json', 'w+', encoding='utf-8')
        file.write(newcons)
        file.close()
        # 替换域名
    ...
    def login(self):
        ...
        if check:
            ...
            url='http://127.0.0.1:'+str(self.port)+'/v2/login?qr-format=base64&result-output=%2Fweb%2Fpython%2Floginresult%2F'+str(self.appid)+'%2Floginresult.json'
            r=requests.get(url)
    ...
    def preview(self):
        self.checkplu()
        self.updates()
        projecturl = urllib.parse.urlencode(self.dir)
    ...
    def upload(self):
        ...
        if not os.path.exists(projecturl+'/app.json'):
            print('json不存在')
...
def mains():
    ...
    http_url = request.args.get("http_url")
    pid = request.args.get("pid")
    version = request.args.get("version")
    desc = request.args.get("desc")
    is_plugins = request.args.get("is_plugins")
    if is_plugins:
        print('需要插件')
    else:
        is_plugins=0
